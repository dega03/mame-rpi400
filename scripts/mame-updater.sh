        ...
        if [ -x $MAMESRCPATH/mame ]; then
            ...
            if [ ! -d /usr/share/fonts/truetype/liberation-fonts ]; then
              ...
              if [ -f liberation_sans.zip ]; then
                ...
                sudo unzip -o ~/liberation_sans.zip *.ttf -d /usr/share/fonts/truetype/liberation-fonts
            ...
            if [ -z $FRONTEND ]; then
              if [ ! -f ~/settings ]; then
                echo FRONTEND=mame>~/settings
              else
                grep -q FRONTEND= ~/settings && sed -i "s/^FRONTEND=.*$/FRONTEND="${1,,}"/g" $(readlink ~/settings) || (echo AUTOROM="${2,,}" | tee -a ~/settings; echo | tee -a ~/settings)
              fi
            fi

            if [ ! -L ~/settings ] && [[ $(findmnt -n /data) ]]; then    # Symlink not found and /data is mounted...
              [ ! -f /data/.sys/env/settings ] && mkdir -p /data/.sys/env
              if [ ! -f ~/settings ]; then
                touch /data/.sys/env/settings
                echo 'FRONTEND=mame'>~/settings
              else      # File settings already exist
                mv ~/settings /data/.sys/env
              fi
              ln -s /data/.sys/env/settings ~/settings
            fi
